...
// 応答フレームのUSB送信
void FRM_MakeAndSendResFrm(USHORT seqNo, USHORT cmd, USHORT errCode,
                           USHORT dataSize, PVOID pBuf) {
    ULONG frmSize;                   // 応答フレームのサイズ(チェックサム除く)
    UCHAR *pDataAry = (UCHAR *)pBuf; // 応答フレームのデータ部
    ST_FRM_RES_FRAME stResFrm = {0}; // 応答フレーム
    ...
    frmSize = offsetof(ST_FRM_RES_FRAME, dataSize) + sizeof(stResFrm.dataSize) +
              stResFrm.dataSize;
    // チェックサムを計算
    ...
    FRM_ReqToSend(
        &stResFrm,
        frmSize); // ヘッダ部～データ部
                  // ※データ部:aData[]メンバについてはfrmSize分だけが送信対象
    FRM_ReqToSend(&stResFrm.checksum,
                  sizeof(stResFrm.checksum)); // チェックサム部
}
