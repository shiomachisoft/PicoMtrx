...
// Critical Section API for short-lived mutual exclusion safe for IRQ and
// multi-core. mutex: Mutex API for non IRQ mutual exclusion between cores.
...
{
    return (CMN_CalcChecksum(pBuf, size) == expect);
}
